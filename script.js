...
// State
let displayOrder = originalData.map(q => q.id);
let currentIndex = 0;
let hiddenIds = new Set();
let pinnedIds = new Set();
let showHiddenInSidebar = false;
...
function getVisibleOrder() {
    return displayOrder.filter(id => !hiddenIds.has(id));
}

function updateUI() {
    const visibleOrder = getVisibleOrder();
    
    // Safety check if everything is hidden
    if (visibleOrder.length === 0) {
        questionText.textContent = "All questions are hidden. Reset or unhide to continue.";
        answerText.textContent = "";
        progressText.textContent = "0 / 0";
        photo.src = "https://i.pravatar.cc/300?u=ghost";
        return;
    }

    // Wrap index if out of bounds (looping)
    if (currentIndex >= visibleOrder.length) currentIndex = 0;
    if (currentIndex < 0) currentIndex = visibleOrder.length - 1;

    const currentId = visibleOrder[currentIndex];
    const data = originalData.find(q => q.id === currentId);
    ...
    progressText.textContent = `${currentIndex + 1} / ${visibleOrder.length}`;
    
    // Update active states for buttons
    pinBtn.classList.toggle('active', pinnedIds.has(currentId));
    hideBtn.classList.toggle('active', hiddenIds.has(currentId));
...
function renderSidebar() {
    ...
    // Pinned questions first, then others
    const sortedIds = [...displayOrder].sort((a, b) => {
    ...
    sortedIds.forEach(id => {
        ...
        if (isHidden && !showHiddenInSidebar) return;

        const data = originalData.find(q => q.id === id);
        const li = document.createElement('li');
        li.className = 'question-item';
        if (id === currentId) li.classList.add('active');
        if (pinnedIds.has(id)) li.classList.add('pinned');
        if (isHidden) li.classList.add('hidden-item');

        li.innerHTML = `
            <span class="q-title">${id}. ${data.question}</span>
            <span>${pinnedIds.has(id) ? '📌' : ''}${isHidden ? '🚫' : ''}</span>
        `;

        li.onclick = () => {
            const indexInVisible = visibleOrder.indexOf(id);
            if (indexInVisible !== -1) {
                currentIndex = indexInVisible;
                updateUI();
            } else if (isHidden) {
                // If clicking a hidden item in sidebar, unhide it first
                hiddenIds.delete(id);
                currentIndex = getVisibleOrder().indexOf(id);
                updateUI();
            }
        };
...
pinBtn.addEventListener('click', () => {
    ...
    if (pinnedIds.has(currentId)) {
        pinnedIds.delete(currentId);
    } else {
        pinnedIds.add(currentId);
    }
    updateUI();
});
...
hideBtn.addEventListener('click', () => {
    ...
    // Move to next available if possible
    if (getVisibleOrder().length > 0) {
        updateUI(); 
    } else {
        updateUI();
    }
...
resetOrderBtn.addEventListener('click', () => {
    displayOrder = originalData.map(q => q.id);
    hiddenIds.clear();
    pinnedIds.clear();
    currentIndex = 0;
    updateUI();
...
showHiddenBtn.addEventListener('click', () => {
    showHiddenInSidebar = !showHiddenInSidebar;
    showHiddenBtn.textContent = showHiddenInSidebar ? '🙈' : '👁️';
    renderSidebar();
});
